...
public class search_follwing_adapter extends RecyclerView.Adapter<search_follwing_adapter.image_board_ViewHolder> {
    private Context context;
    private List<login_data> lists;

    public search_follwing_adapter(Context context, List<login_data> lists ) {
    ...
    @Override
    public search_follwing_adapter.image_board_ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.friends_record_item, parent, false);
        return new search_follwing_adapter.image_board_ViewHolder(view);
    }


    @Override
    public void onBindViewHolder(search_follwing_adapter.image_board_ViewHolder holder, int position) {
        login_data data = lists.get(position);


        holder.friends_record_item_textview.setText(data.getOther_nick());

        System.out.println(data.getOther_image()+"data.getOther_image().equals(\"\")");
        if (data.getOther_image().equals("")) {
            ...
            Glide.with(context)
                    .load("http://52.79.180.89/kakao_story" + data.getOther_image())
